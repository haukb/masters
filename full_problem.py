...
class Full_problem:
    ...
    def _build_variables(self):
        m = self.m
        #Fetch the scenario for which to solve the full problem
        S = self.data.S
        ...
        self.variables.vessels = m.addVars(product(V,N_s[S[0]]), vtype=GRB.INTEGER, name="Vessel investment")
        self.variables.ports = m.addVars(product(P,N_s[S[0]]), vtype=GRB.BINARY, name="Port investment")
    ...
    def _build_constraints(self):
        ...
        gp.quicksum(DELIVERY[k,t,w,s] for k in K_i[i])*gp.quicksum(BETA.iloc[n,t]*ports[(i,n)] for n in N_s[s]) 
        ...
        gp.quicksum(PICKUP[k,t,w,s] for k in K_i[i])*gp.quicksum(BETA.iloc[n,t]*ports[(i,n)] for n in N_s[s]) 
        for i in P[1:] for t in T for w in W for s in S)
        ...
        self.constraints.c13 = m.addConstrs(gp.quicksum(BETA.iloc[n,t]*vessels[(v, n)] for n in N_s[s]) >= \
    ...
    def _build_objective(self):
        ...
        total_costs = gp.quicksum(
        #Investment cost
        gp.quicksum(gp.quicksum(VESSEL_INVESTMENT[v,n]*vessels[(v, n)] for v in V) + gp.quicksum(PORT_INVESTMENT[i,n]*ports[(i, n)] for i in P) for n in N_s[s]) \
        #Vessel costs
        + (52/NUM_WEEKS) * gp.quicksum(gp.quicksum(SAILING_COST[v,r,t]*routes_vessels[(v,r,t,s)] + \
        ...
        #Truck costs
        + (52/NUM_WEEKS) * gp.quicksum(TRUCK_COST[i,k,t,s]*(delivery_truck[(i,k,t,w,s)]+pickup_truck[(i,k,t,w,s)]) for k in K for i in P_k[k] for t in T for w in W) \
        for s in S)

...
class Subproblem_adhoc(Subproblem):
    ...
    def _save_vars(self):
        ...
        sens_routes_vessels = [
            dict.fromkeys(product(R_v[v], self.mp.data.T, self.mp.data.S), [])
            for v in V
        ]
    ...
    def _add_route_fixation_constraint(self):
        ...
        self.constraints.fix_routes_vessels = self.m.addConstrs(
            routes_vessels_free[(v, r, t, s)] == 0
            for v in V
            for r in R_v[v]
            for t in self.mp.data.T
            for s in self.mp.data.S
        )
            ...
            for r in self.mp.data.R_v[v]:
                for t in self.mp.data.T:  # _n[self.NODE]:
                    for s in self.mp.data.S:  # _n[self.NODE]:
                        routes_vessels_val = updated_data.routes_vessels[(v, r, t, s)][
                            -1
                        ]

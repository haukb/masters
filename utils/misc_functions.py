...
def nodes_with_new_investments(
    ...
    for n in N:
        ...
        cum_ports_latest = np.array(
            [sum([ports[(i, m)][-1] for m in NP_n[n]]) for i in P]
        )
        mp2sp_iterations[mp_iter, n] = (
            max(mp2sp_iterations[:, n]) + 1
        )  # Default option, given that node solution is new

        for j in range(mp_iter):  # The number of MP iterations
            cum_vessels = np.array(
                [sum([vessels[(v, m)][j] for m in NP_n[n]]) for v in V]
            )
            cum_ports = np.array([sum([ports[(i, m)][j] for m in NP_n[n]]) for i in P])
            if (cum_vessels_latest == cum_vessels).all() and (
                cum_ports_latest == cum_ports
            ).all():
                N_changed.remove(n)

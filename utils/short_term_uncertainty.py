...
def draw_weekly_demand(baseline_demand, customer_type):
    ...
    if customer_type == "Consumer":
        random_demand = np.random.normal(
            loc=baseline_demand, scale=baseline_demand * 0.64
        )

    elif customer_type == "Industrial":
        random_demand = np.random.normal(
            loc=baseline_demand, scale=baseline_demand * 0.1
        )

    return max(
        min(random_demand, baseline_demand * 2), 0
    )  # ensures that demand can't be negative
...
def generate_random_weeks(instances, SCENARIOS):
    ...
    for inst in instances:
        CUSTOMER_DATA = pd.read_csv(
            f"Data/Instances/{inst}/Input_data/Customer_Data.csv", index_col=0
        )
        SCENARIOYEAR_GROWTH_FACTOR = pd.read_csv(
            f"Data/Nodes_and_scenarios/ScenarioYear_Growth_Factor_{SCENARIOS}.csv",
            index_col=0,
        )
        ...
        for c in K:
            ...
            for w in W:
                for t in T:
                    weekly_variation_factor[c, w, t] = draw_weekly_demand(
                        1, customer_type
                    )
...
if __name__ == "__main__":
    ...
    INSTANCES = []  # This line choses for which instance to generate new random weeks
    SCENARIOS = 27
